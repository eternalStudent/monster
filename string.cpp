...
int32 uint32ToDecimal(uint32 number, _Out_ char* str) {
...
int32 int32ToDecimal(int32 number, _Out_ char* str) {
	if (number < 0) {
		*str = '-';
...
int32 float32ToDecimal(float32 number, int32 precision, _Out_ char* str) {
	int32 count = 0;
...
#define CopyString(source, byteToCopy, dest) (memcpy((dest), (source), (byteToCopy)), (byteToCopy))

inline int32 boolToAnsi(bool b, char* str){
	if (b){
		return CopyString("true", 5, str) - 1;
	}
	else{
		return CopyString("false", 6, str) - 1;
	}
}