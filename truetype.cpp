...
int32 GetGlyphShapeTT(const FontInfo* info, int32 glyph_index, vertex** pvertices) {
   ...
   *pvertices = NULL;
   ...
   if (numberOfContours > 0) {
      ...
      vertices = (vertex *) Alloc(m * sizeof(vertices[0]));
      if (vertices == 0)
         return 0;
      ...
      while (more) {
         ...
         if (comp_num_verts > 0) {
            ...
            // Append vertices.
            tmp = (vertex*)Alloc((num_vertices+comp_num_verts)*sizeof(vertex));
            if (!tmp) {
...
int32 GetGlyphShapeT2(const FontInfo* info, int32 glyph_index, vertex** pvertices) {
   // runs the charstring twice, once to count and once to output (to avoid realloc)
   csctx count_ctx = CSCTX_INIT(1);
   csctx output_ctx = CSCTX_INIT(0);
   if (run_charstring(info, glyph_index, &count_ctx)) {
      *pvertices = (vertex*)Alloc(count_ctx.num_vertices*sizeof(vertex));
...
void MakeGlyphBitmapSubpixel(const FontInfo* info, byte* output, 
...
void MakeGlyphBitmap(const FontInfo* info, byte* output, 
                     int32 out_w, int32 out_h, int32 out_stride, 
