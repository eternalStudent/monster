...
void *hheap_alloc(hheap *hh, ssize size) {
   if (hh->first_free) {
      ...
      if (hh->num_remaining_in_head_chunk == 0) {
         ...
         hheap_chunk *c = (hheap_chunk *) Alloc(sizeof(hheap_chunk) + size * count);
         if (c == NULL)
            return NULL;
         c->next = hh->head;
         hh->head = c;
...
void rasterize(Image* result, Point2 *pts, int32 *wcount, 
   ...
   e = (edge *) Alloc(sizeof(*e) * (n+1)); // add an extra one as a sentinel
   if (e == 0) return;
   n = 0;
...
Point2 *FlattenCurves(vertex *vertices, int32 num_verts, float32 objspace_flatness, int32 **contour_lengths, int32 *num_contours)
{
   Point2 *points=0;
   int32 num_points=0;
   ...
   *contour_lengths = (int32 *) Alloc(sizeof(**contour_lengths) * n);
   ...
   for (pass=0; pass < 2; ++pass) {
      float32 x=0,y=0;
      if (pass == 1) {
         points = (Point2 *) Alloc(num_points * sizeof(points[0]));
...
void Rasterize(Image* result, float32 flatness_in_pixels, vertex *vertices, int32 num_verts, float32 scale_x, float32 scale_y, float32 shift_x, float32 shift_y, int32 x_off, int32 y_off, int32 invert)
{
   ...
   Point2 *windings = FlattenCurves(vertices, num_verts, flatness_in_pixels / scale, &winding_lengths, &winding_count);
   if (windings) {
      rasterize(result, windings, winding_lengths, winding_count, scale_x, scale_y, shift_x, shift_y, x_off, y_off, invert);
